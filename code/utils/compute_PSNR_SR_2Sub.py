...
method_sr = ['EDSR']
len_m = len(method_sr)
train_flag = [[v+'_CONTINUE', 'FINETUNE_'+v] for v in method_sr]
psnr_flag = ['_WON_Solo', '_WN_Solo']#, '_WN_Solo_E100']
len_f = len(psnr_flag)
...
with open('./results/ALL_PSNR.pkl', 'rb') as f:
    psnr_up = pkl.load(f)
method_all = psnr_up['method'] + [method_sr[m]+psnr_flag[f] for m in range(len_m) for f in range(len_f)]
num_pre = len(psnr_up['method'])

for d in dataset:
    img_hr_list = glob.glob(os.path.join(path, d, 'x1/Texture/*.png'))
    num = len_m * len_f
    psnr_ys = np.zeros([len(img_hr_list)+1, num_pre+num, 3])
    psnr_ys[:, :num_pre, :] = psnr_up['psnr_all_y'][dataset.index(d)]
    psnr_s = np.zeros([len(img_hr_list)+1, num_pre+num, 3])
    psnr_s[:, :num_pre, :] = psnr_up['psnr_all'][dataset.index(d)]
        ...
        for s in range(2, 5):
            for m in range(len_m):
                for f in range(len_f):
                    ...
                    t_flag = train_flag[m][1] if f == 2 else train_flag[m][f]
                    img_sr_n = os.path.join(path_sr, method_sr[m]+'_X{}_'.format(s)+t_flag+'_'+flag, 'results',
                                           name_img+'_x{}_'.format(s)+t_flag+'.png')
    ...
    save_html([os.path.splitext(os.path.basename(n))[0] for n in img_hr_list], method_all, psnr_ys, d, './results/TOTAL_PSNR_Y.html')
    save_html([os.path.splitext(os.path.basename(n))[0] for n in img_hr_list], method_all, psnr_s, d, './results/TOTAL_PSNR_RGB.html')

with open('./results/TOTAL_PSNR.pkl', 'wb') as f:
...
save_html(dataset, method_all, psnr_sm_y, 'All', './results/TOTAL_Summary_Y.html')
save_html(dataset, method_all, psnr_sm, 'All', './results/TOTAL_Summary_RGB.html')
